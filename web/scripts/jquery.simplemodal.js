...
(function($) {
	var ie6 = $.browser.msie && parseInt($.browser.version) == 6
			&& !window['XMLHttpRequest'], ieQuirks = $.browser.msie
			&& !$.boxModel, w = [];
	...
	$.modal.impl = {
		...
		init : function(data, options) {
			...
			this.dialog.data = data.addClass('simplemodal-data').css(
					this.opts.dataCss);
			data = null;
			this.create();
			this.open();
			...
			this.dialog.container = $('<div/>').attr('id',
					...
					.css($.extend(this.opts.containerCss, {
						...
						zIndex : this.opts.zIndex + 2
							...
							this.opts.close ? $(this.opts.closeHTML).addClass(
		...
		setPosition : function() {
			var top, left, hCenter = (w[0] / 2)
					- ((this.dialog.container.height() || this.dialog.data
							.height()) / 2), vCenter = (w[1] / 2)
